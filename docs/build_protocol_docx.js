    ...
    children: [
      ...
      table(
        ...
        [
          ...
          ['/show/heartbeat', 'seq : int\nuptime : float', '1 Гц — пульс мастера,\nseq растёт каждую секунду'],
        ],
        [3100, 3400, 3580], [0, 1]),
      p(''),
      ...
      table(
        ...
        [
          ...
          ['/zoneN/time', 'elapsed : float', '1 Гц — сколько идёт СЦЕНА ЭТОЙ ЗОНЫ'],
      ...
      p('Три разных времени, не путать:', { bold: true, after: 100 }),
      bullet('/show/clock — общие часы шоу, 30 Гц, одни на всю инсталляцию.'),
      bullet('/zoneN/time — сколько идёт сцена ЭТОЙ зоны от её собственного старта, 1 Гц: вычитать ничего не нужно.'),
      bullet('/show/heartbeat — пульс мастера с растущим seq: пропал — мастер недоступен, пошёл с единицы — мастер перезапустился (по потоку часов пауза и рестарт неотличимы).'),
      p(''),
      p('Начало отсчёта:', { bold: true, after: 100 }),
      rich([
        { t: 'zx_rel = 0', mono: true },
        { t: ' — там, где гость ВХОДИТ в зону, ' }, { t: '1', mono: true },
        { t: ' — где выходит. Движение справа налево, поэтому нулю соответствует правый край зоны; мастер разворачивает шкалу сам, зоне об этом думать не нужно. ' },
        { t: 'zy_rel', mono: true }, { t: ' — поперёк тоннеля, 0 у края y = 0. Переключается парой ' },
        { t: 'Relorigin', mono: true }, { t: ' на zones: entry (по умолчанию) или corner — от угла зоны.' },
      ], { after: 160 }),
      p('Про позиции — главное:', { bold: true, after: 100 }),
      bullet('zx_rel — 0 на входе в зону, 1 на выходе; zy_rel — поперёк тоннеля 0…1.'),
